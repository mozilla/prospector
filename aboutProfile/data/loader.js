...
try {
	...
	var snippets = $(".snippet.snippet-medium");

	console.log( "SIZE " + snippets.length );
	snippets.each( function( i , element ) {
	    //this.("a");
		//let a = this.getElementByTagName( "a" );
		//let img = this.getElementByTagName( "img" );
		let ref = $(element).find("a").first().attr( "href" );
		let img = $(element).find("img").first().attr( "src" );
		let title = $(element).find("img").first().attr( "alt" );
		let descr = $(element).find("p").first().text( );

        self.port.emit(  "found" , ref , img , title , descr );
	});
} catch (ex) {
...
self.port.on ( "show_controls", function ( limit , names , currentName , depth , frecency , days , cutOff ) {
  ...
  names.forEach( function( item ) {
	...
	if( item == currentName ) nd.attr( 'selected' , '1' );
...
try {
    ...
    Object.keys( cats ).sort( function( a, b ) {
      ...
      let catNode = $("<cpan/>").addClass( "cat" ).append(
            ...
            $("<div/>").addClass( "bar" ).text("_").css( {"width" : barWidth+"px"}) ,
            $("<span/>").addClass( "bar_number" ).css({ "font-size" : "x-small"}).text( value )
...
self.port.on ( "show_cats", function ( cats , totalAcross ) {
	...
	var cutOff = $("#catCutOff").attr( "value" ) * 1.0 * totalAcross ;
	var fullPath = $("#fullPath").attr( "checked" );
	...
	Object.keys( catBukets ).sort( 
		function( a , b ) {
			return catBukets[ b ].total - catBukets[a].total;
		}).forEach( function( top ) {
			console.log( top , JSON.stringify(catBukets[top].cats));
			catNames = catNames.concat( catBukets[top].cats.sort( function ( a , b ) { return cats[b].vcount - cats[a].vcount; }));
		});
    } else {
    ...
    for( x in catNames ) {
	  ...
	  if( ! largest ) largest = catData.vcount;
	  ...
	  let catNode = $("<cpan/>").addClass( "cat" ).append( 
	  		...
	  		$("<div/>").addClass( "bar" ).text("_").css( {"width" : barWidth+"px"}) ,
			$("<span/>").addClass( "bar_number" ).css({ "font-size" : "x-small"}).text( Math.round(catData.vcount) )
...
function displayDemogs( demog , buketNames ) {
	...
	for( x in buketNames ) {
		...
		let theNode = $("<cpan/>").addClass( "demog" ).append(
			...
			$("<span/>").addClass( "bar_number" ).css({ "font-size" : "x-small"}).text( Math.floor( vtotal ))
		);
        ...
        for ( x in champs ) {
         negNode.append( $("<li/>").css( { "color" : "blue" }).text( champs[x].item.domain + " " + Math.round(champs[x].item.vcount) + " " + champs[x].item.drop ) );
        }
        ...
        for ( x in champs ) {
         negNode.append( $("<li/>").css( { "color" : "olive" }).text( champs[x].item.domain + " " + Math.round(champs[x].item.vcount) + " " + champs[x].item.drop ) );
        }
